...
class FerrariChatScraper:
        ...
        while True:
            try:
                ...
                for result in search_results:
                    ...
                    timestamp = (
                        timestamp_element.get("title", "") if timestamp_element else ""
                    )
                    ...
                    snippet = (
                        snippet_element.find("a").text
                        if snippet_element and snippet_element.find("a")
                        else ""
                    )
    ...
    def _extract_post_data(self, post_element):
        ...
        try:
            ...
            created_date = (
                created_date_element.find("dd").string if created_date_element else ""
            )
            ...
            if content_element:
                ...
                # Get attached files
                attached_files = []
                attached_files_header = content_element.find(
                    "h4", class_="attachedFilesHeader"
                )
                if attached_files_header:
                    attached_files_list = content_element.find(
                        "ul", class_="attachmentList"
                    )
                    if attached_files_list:
                        for file_item in attached_files_list.find_all(
                            "li", class_="attachment"
                        ):
                            file_name = (
                                file_item.find("h6", class_="filename").text
                                if file_item.find("h6", class_="filename")
                                else ""
                            )
