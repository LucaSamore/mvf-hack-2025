...
def process_comments_in_chunks(comments, sentiment_pipeline, chunk_size=1000):
            ...
            if "comment" in comment:
                try:
                    ...
                    sentiment = analyze_sentiment(
                        [comment["comment"]], sentiment_pipeline
                    )[0]
                    ...
                    processed_comment["sentiment"] = {"label": "ERROR", "score": 0.0}
    ...
    if all_sentiments:
        positive_ratio = sum(
            1 for s in all_sentiments if s["label"] == "POSITIVE"
        ) / len(all_sentiments)
        negative_ratio = sum(
            1 for s in all_sentiments if s["label"] == "NEGATIVE"
        ) / len(all_sentiments)
...
def main():
    try:
        ...
        if not hf_token:
            raise ValueError(
                "HUGGINGFACE_TOKEN not found in environment variables. Please set it in .env file"
            )
        ...
        logger.info(
            f"Comments skipped: {stats['total_processed'] - stats['total_analyzed']}"
        )
